...
import { debug } from "../../utils/debug"; // Corrected path
import {
  MicrophoneIcon,
  StopIcon,
  StarIcon,
...
export const OrderInterface: React.FC = () => {
  ...
  const locationState = location.state as LocationState | null; // Type assertion
  ...
  // Effect to initialize state from navigation
  useEffect(() => {
    if (locationState?.tableId && locationState?.seatId) {
      debug.info("Received table/seat state from navigation", DEBUG_OPTIONS);
      debug.logState({ navState: locationState }, DEBUG_OPTIONS);
      setOrderType("table"); // Ensure order type is table
      setSelectedTable(locationState.tableId);
      // Use seatNumber from state for display consistency if available, otherwise seatId
      setSelectedSeat(locationState.seatNumber || locationState.seatId);
      // Optionally, clear room number if navigating from table view
      setRoomNumber("");
    }
  }, [locationState]); // Re-run if navigation state changes

  // Get current meal period
  const { data: mealPeriodData, isLoading: isLoadingMealPeriod } =
    useQuery<string>({
  ...
  const { data: tables, isLoading: isLoadingTables } = useQuery<Table[]>({
    ...
    queryFn: async () => {
      ...
      // Use /api/tables/layout as it returns the full data needed
      const response = await api.get("/api/tables/layout");
      debug.info(
        "Tables fetched successfully using /layout endpoint",
        DEBUG_OPTIONS,
      );
      // Assuming the layout endpoint returns { tables: [...] }
  ...
  const {
    ...
    error: dailySpecialError,
  } = useQuery({ // Removed <DailySpecial> type annotation
  ...
  const createOrderMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tables"] }); // Maybe invalidate orders too?
      setOrderText("");
      setError(null);
      // Clear selection if needed, or navigate back?
  ...
  useEffect(() => {
    const getQuickOrderOptions = async () => {
      if (selectedTable && selectedSeat && currentMealPeriod) {
        try {
          debug.logApiCall(
            `/api/quick-orders/table/${selectedTable}/seat/${selectedSeat}?meal_period=${currentMealPeriod}`,
            "GET",
            {},
            DEBUG_OPTIONS,
          );
    ...
    try {
      ...
      mediaRecorder.ondataavailable = async (event) => {
        if (event.data.size > 0) {
          ...
          formData.append("text", orderText); // Include current text context if needed

          try {
            // TODO: Ensure this endpoint exists and accepts POST
            debug.logApiCall(
            ...
            const response = await api.post(
              "/api/orders/transcribe", // Use relative path with api util
              formData,
              {
                headers: {
  ...
  // handleDailySpecialSelect removed

  if (isLoadingTables) {
    // Check loading state for tables
    return (
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">
            Place Order{" "}
            {locationState?.tableNumber && locationState?.seatNumber
              ? `for Table ${locationState.tableNumber}, Seat ${locationState.seatNumber}`
              : ""}
          </h2>
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg">
            <ClockIcon className="h-5 w-5 text-blue-500" />
            <span className="text-blue-700 font-medium capitalize">
              {currentMealPeriod} Service
            </span>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <button
                ...
                disabled={!!locationState?.tableId} // Disable if navigated here for a specific table
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  required
                  disabled={!!locationState?.tableId} // Disable if table is passed via navigation state
                >
                  {/* Show selected table if passed via state, otherwise show options */}
                  {locationState?.tableId && locationState?.tableNumber ? (
                    <option value={locationState.tableId}>
                      Table {locationState.tableNumber}
                    </option>
                  ) : (
                    <>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 disabled:bg-gray-100"
                  required
                  disabled={!selectedTable || !!locationState?.seatId} // Disable if no table selected OR if seat passed via navigation state
                >
                  {/* Show selected seat if passed via state, otherwise show options */}
                  {locationState?.seatId && locationState?.seatNumber ? (
                    <option value={locationState.seatId}>
                      Seat {locationState.seatNumber}
                    </option>
                  ) : (
                    <>
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                {isRecording ? (
                  ...
                  <button
                    ...
                    aria-label="Start Recording" // Add aria-label for accessibility and testing
                  >
