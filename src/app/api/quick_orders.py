...
from src.app.utils.meal_utils import (
    get_frequent_orders,
    get_resident_for_seat,
)  # Removed get_daily_special

router = APIRouter(tags=["quick-orders"])  # REMOVED prefix
...
@router.get(
    "/table/{table_id}/seat/{seat_number}", response_model=QuickOrderOptionsResponse
)
async def get_quick_order_options(
    table_id: int,
    seat_number: int,
    # meal_period: str = Query(..., description="Current meal period (breakfast, lunch, dinner)"), # Removed
    ...
    Args:
        ...
        # meal_period: The meal period (breakfast, lunch, dinner) # Removed
    ...
    # Get frequent orders for this resident (removed meal_period filter)
    frequent_orders = get_frequent_orders(
        db, resident.id, limit=3
    )  # Removed meal_period argument
    ...
    if resident:
        resident_info = {
            ...
            "name": resident.name,
            "medical_dietary": (
                resident.medical_dietary if resident.medical_dietary else []
            ),
