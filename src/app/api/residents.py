...
from src.app.models.models import Resident, Order
from src.app.api.schemas import (
    ResidentCreate,
    ResidentUpdate,
    Resident as ResidentSchema,
)
...
def update_resident(
    resident_id: int, resident: ResidentUpdate, db: Session = Depends(get_db)
):
    """
    Update a resident
    """
...
def delete_resident(resident_id: int, db: Session = Depends(get_db)):
    ...
    order_count = (
        db.query(func.count(Order.id)).filter(Order.resident_id == resident_id).scalar()
    )
    if order_count > 0:
        raise HTTPException(
            status_code=400,
...
def get_resident_orders(
    resident_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)
):
    """
    ...
    orders = (
        db.query(Order)
        .filter(Order.resident_id == resident_id)
