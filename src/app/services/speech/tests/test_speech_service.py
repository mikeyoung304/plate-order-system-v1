...
from src.app.services.speech.deepgram_service import SpeechService
...
async def test_process_audio_success(
    speech_service, mock_audio_data, mock_transcription
):
    with patch(
        "app.services.voice.deepgram_service.DeepgramService.transcribe_audio"
    ) as mock_transcribe:
...
async def test_process_audio_recording_success(speech_service, mock_transcription):
    with (
        patch(
            "app.services.voice.audio_recorder.AudioRecorder.record_audio"
        ) as mock_record,
        patch(
            "app.services.voice.deepgram_service.DeepgramService.transcribe_audio"
        ) as mock_transcribe,
...
async def test_validate_setup_success(speech_service):
    with (
        patch(
            "app.services.voice.deepgram_service.DeepgramService.validate_api_key"
        ) as mock_validate,
        patch(
            "app.services.voice.audio_recorder.AudioRecorder.list_audio_devices"
        ) as mock_devices,
    ):
