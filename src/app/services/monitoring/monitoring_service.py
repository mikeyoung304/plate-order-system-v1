...
class MonitoringService:
    ...
    async def record_transcription_metrics(
        ...
        if error:
            await self.metrics_client.record_metric(
                "transcription.error", {"error": error, "timestamp": time.time()}
            )
    ...
    async def record_order_metrics(
        ...
        if error:
            await self.metrics_client.record_metric(
                "order.error", {"error": error, "timestamp": time.time()}
            )
    ...
    async def record_rate_limit_hit(self, endpoint: str, client_ip: str):
        ...
        await self.metrics_client.record_metric(
            "rate_limit.hits",
            {"endpoint": endpoint, "client_ip": client_ip, "timestamp": time.time()},
        )
            ...
            async def wrapper(*args, **kwargs) -> T:
                try:
                    ...
                    await self.record_error(
                        operation_name, str(e), {"function": func.__name__}
                    )
                    raise
