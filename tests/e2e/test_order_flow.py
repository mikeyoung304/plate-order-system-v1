...
from app.api.schemas import OrderCreate, Order as OrderSchema
...
async def test_multiple_orders_flow(test_client, sample_audio_data, order_processor):
    ...
    response = await test_client.post(
        f"/orders/{first_order['order_id']}/confirm",
        json={"confirmed": True}
    )
    assert response.status_code == 200
    ...
    response = await test_client.post(
        f"/orders/{second_order['order_id']}/confirm",
        json={"confirmed": False}
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_rate_limit_recovery_flow(test_client, sample_audio_data, order_processor):
