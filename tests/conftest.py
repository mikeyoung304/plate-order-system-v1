...
from app.services.speech.deepgram_service import DeepgramService
...
async def test_client():
    ...
    async with AsyncClient(app=app, base_url=f"http://test{test_settings.API_V1_STR}") as client:
        yield client
...
async def order_processor():
    ...
    with patch('app.services.order.order_processor.DeepgramService') as mock_deepgram, \
         patch('app.services.order.order_processor.MonitoringService') as mock_monitoring:
...
async def deepgram_service():
    ...
    with patch('app.services.speech.deepgram_service.settings.DEEPGRAM_API_KEY', 'test_key'):
