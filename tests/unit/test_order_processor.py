...
async def test_process_order_no_items(order_processor, sample_audio_data):
    ...
    order_processor.deepgram.transcribe_audio.return_value = {
        "results": {
            "channels": [{
                "alternatives": [{
                    "transcript": "I would like something else",
                    "confidence": 0.9
                }]
            }]
        }
...
async def test_process_order_error(order_processor):
    ...
    order_processor.deepgram.transcribe_audio.side_effect = Exception("Transcription failed")
...
async def test_confirm_order_error(order_processor):
    ...
    # Configure mock to raise exception
    order_processor.monitoring.record_order_metrics.side_effect = Exception("Metrics error")
