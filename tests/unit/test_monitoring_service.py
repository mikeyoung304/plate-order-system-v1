...
async def test_record_transcription_error(monitoring_service):
    ...
    error_call = [
        call
        for call in monitoring_service.metrics.record_metric.call_args_list
...
async def test_record_order_metrics(monitoring_service):
    ...
    # Record metrics
    await monitoring_service.record_order_metrics(
        processing_time, success, num_items=items_count
    )
